    ...
    <div class="row">
        <div class="col-sm-4 col-sm-offset-4">
            ...
            if(isset($_POST['submit'])) {
                $database->query('SELECT id FROM users WHERE token = ?');
                $id = $database->fetch([getToken()])['id'];
                if(empty($_POST['title']) || empty($_POST['body'])) {
                    echo 'Post Failed';
                }
                else {
                    $database->query('INSERT INTO posts (users_id, title, body, url) VALUES (?, ?, ?, SHA(NOW()))');
                    $database->execute([$id, $_POST['title'], $_POST['body']]);
                    $users_id = $database->lastInsertId();
                    if(isset($_POST['tag'])) {
                        foreach ($_POST['tag'] as $value) {
                            $database->query('INSERT INTO posts_tags (posts_id, tags_id) VALUES (?, ?)');
