...
class Post {
    ...
    public function __construct($id = null, $title = null, $body = null, $date = null, $url = null, $author_id = null) {
        ...
        $database->query('SELECT id, firstname, lastname, email, url FROM users WHERE id = ? LIMIT 1');
        $row = $database->fetch([$author_id]);
        $this->author = new User($row['id'], $row['firstname'], $row['lastname'], $row['email'], $row['url']);

        $database->query('SELECT t.id, t.name FROM posts p LEFT JOIN posts_tags pt ON p.id = pt.posts_id LEFT JOIN tags t ON pt.tags_id = t.id WHERE p.id = ?');
